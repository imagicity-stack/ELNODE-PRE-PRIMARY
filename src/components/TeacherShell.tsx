...
export default function TeacherShell({ children, user }: { children: React.ReactNode; user: UserProfile }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (auth.currentUser) {
        const teacherId = (auth.currentUser as any).teacherId || auth.currentUser.uid;
        const classIds = user.classId ? [user.classId] : [];
        unsub = startNotificationListeners(
          teacherId, 'teacher', classIds,
  ...
  const handleLogout = async () => {
    try {
      await logActivity(user, 'User Logged Out', 'Teachers', `${user.name} signed out of the Teachers portal`);
    } catch { /* non-fatal */ }
    await signOut(auth);
    navigate('/login');
  };
        ...
        <div className="p-3 space-y-1" style={{ borderTop: '1px solid var(--line)' }}>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="eyebrow mt-0.5">Teacher</p>
            </div>
          </Link>
          <button
